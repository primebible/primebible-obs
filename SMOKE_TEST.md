...
Verify server starts successfully and the banner shows:
```
...
- ✅ Server console shows: `[OBS] Connected`
...
**Objective:** Verify PIN protection for remote and control access
...
**Expected:**
- ✅ WebSocket connection rejected (close code 4001)
- ✅ Page prompts for the PIN
- ✅ Entering the wrong PIN re-prompts; entering `1234` connects
- ✅ Same behavior on `http://localhost:4456/control`

5. Access with PIN in the URL: `http://localhost:4456/remote?pin=1234`

**Expected:**
- ✅ Connection succeeds without a prompt
- ✅ PIN is remembered on that device (localStorage)

6. Mutating REST calls require the PIN too:
   ```bash
   curl -X DELETE http://localhost:4456/api/history          # 401
   curl -X DELETE "http://localhost:4456/api/history?pin=1234"  # ok
   ```

7. Overlay and stage connections also require the PIN. Re-run "Setup Overlay
   in All Scenes" so the OBS source URL picks up the PIN automatically; a
   manually added browser source needs `?pin=1234` appended to its URL.

8. Set `remotePin` back to `""` to disable
...
- ✅ Transition time reduced to 250ms
